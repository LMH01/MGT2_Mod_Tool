...
public class GenreModOld extends AbstractAdvancedModOld {
            ...
            try{
                ...
                while(true){
                    if(JOptionPane.showConfirmDialog(null, params, I18n.INSTANCE.get("dialog.genreHelper.addRandomizedGenre.title"), JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION){
                        ...
                        map.put("ID", Integer.toString(ModManager.genreModOld.getAnalyzer().getFreeId()));
    ...
    /**
     * Sets the genreScreenshots ArrayList a new array list containing the selected files. Uses {@link GenreModOld#getGenreScreenshots()} to get the Array List.
    ...
    private String getRandomGenreCombs(){
        StringBuilder stringBuilder = new StringBuilder();
        for(Integer integer : ModManager.genreModOld.getAnalyzer().getActiveIds()){
    ...
    private List<HashSet<Integer>> getRandomGameplayFeatureIds(){
        ...
        for(String string : ModManager.gameplayFeatureModOld.getAnalyzer().getContentByAlphabet()){
            if(Utils.getRandomNumber(1,100) > 80){
                hashSetBadGameplayFeatures.add(ModManager.gameplayFeatureModOld.getAnalyzer().getContentIdByName(string)-1);
            }
            if(Utils.getRandomNumber(1,100) > 80){
                hashSetGoodGameplayFeatures.add(ModManager.gameplayFeatureModOld.getAnalyzer().getContentIdByName(string)-1);
            }
        }
        ...
        for(Map<String, String> map : ModManager.gameplayFeatureModOld.getAnalyzer().getFileContent()){
            for(Map.Entry<String, String> entry : map.entrySet()){
                for(Integer integer : badGameplayFeatures){
                    if(entry.getKey().equals("ID")){
                        if(entry.getValue().equals(Integer.toString(integer))){
                            gameplayFeaturesBad.append("<").append(ModManager.gameplayFeatureModOld.getAnalyzer().getContentNameById(integer)).append(">");
                ...
                for(Integer integer : goodGameplayFeatures){
                    if(entry.getKey().equals("ID")){
                        if(entry.getValue().equals(Integer.toString(integer))){
                            gameplayFeaturesGood.append("<").append(ModManager.gameplayFeatureModOld.getAnalyzer().getContentNameById(integer)).append(">");
                            if(Settings.enableDebugLogging){
