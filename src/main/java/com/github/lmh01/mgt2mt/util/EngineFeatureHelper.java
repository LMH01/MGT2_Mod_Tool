...
public class EngineFeatureHelper {
    ...
    /**
     * Opens a gui where the user can select the engine feature that should be removed
     */
    public static void removeEngineFeature(){
        try {
            AnalyzeExistingEngineFeatures.analyzeEngineFeatures();
            Backup.createBackup(Utils.getEngineFeaturesFile());
            boolean noEngineFeatureToRemoveAvailable = true;
            JLabel labelChooseEngineFeature = new JLabel("Select the engine feature(s) that should be removed:");
            String[] string;
            if(Settings.disableSafetyFeatures){
                string = AnalyzeExistingEngineFeatures.getEngineFeaturesByAlphabet();
                noEngineFeatureToRemoveAvailable = false;
            }else{
                string = AnalyzeExistingEngineFeatures.getCustomEngineFeaturesString();
                if(string.length != 0){
                    noEngineFeatureToRemoveAvailable = false;
                }
            }
            JList<String> listAvailableEngineFeatures = new JList<>(string);
            listAvailableEngineFeatures.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
            listAvailableEngineFeatures.setLayoutOrientation(JList.VERTICAL);
            listAvailableEngineFeatures.setVisibleRowCount(-1);
            JScrollPane scrollPaneAvailableEngineFeatures = new JScrollPane(listAvailableEngineFeatures);
            scrollPaneAvailableEngineFeatures.setPreferredSize(new Dimension(315,140));

            Object[] params = {labelChooseEngineFeature, scrollPaneAvailableEngineFeatures};

            if(!noEngineFeatureToRemoveAvailable){
                if(JOptionPane.showConfirmDialog(null, params, "Remove Engine feature", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION){
                    if(!listAvailableEngineFeatures.isSelectionEmpty()){
                        boolean exportFailed = false;
                        int numberOfEngineFeaturesToRemove = listAvailableEngineFeatures.getSelectedValuesList().size();
                        StringBuilder failedEngineFeatureRemoves = new StringBuilder();
                        for(int i=0; i<listAvailableEngineFeatures.getSelectedValuesList().size(); i++){
                            ...
                            numberOfEngineFeaturesToRemove--;
                        }
                        if(numberOfEngineFeaturesToRemove == 0){
                            if(exportFailed){
                                JOptionPane.showMessageDialog(null, "Something went wrong wile removing Engine features.\\nThe following Engine features where not removed:\n" + failedEngineFeatureRemoves, "Publisher removal incomplete", JOptionPane.WARNING_MESSAGE);
                            }else{
                                JOptionPane.showMessageDialog(null, "All selected Engine features have been removed successfully!", "Engine features removal successful", JOptionPane.INFORMATION_MESSAGE);
                            }
                        }
                    }else{
